...
async function scrapeState(state) {
  ...
  $('div.row.my-3').each((_, row) => {
    ...
    if (digits.length > 0) results[label] = digits;
  });

  results._date = $('time[datetime]').first().attr('datetime') || '';
...
async function scrapeDraw(scraperConfig, drawConfig) {
  ...
  // Only block future draw times when source data is from today
  // Yesterday's draws have all completed, so skip this check for old data
